...
def createFPtree(dataSet, minSup=1):
    ...
    for tranSet, count in list(dataSet.items()):
        ...
        if len(localD) > 0:
            ...
            orderedItem = [v[0] for v in sorted(iter(localD.items()), key=lambda p:(p[1], int(p[0])), reverse=True)]
            # 用过滤且排序后的样本更新树
...
def mineFPtree(inTree, headerTable, minSup, preFix, freqItemList):
    ...
    bigL = [v[0] for v in sorted(list(headerTable.items()), key=lambda p:p[1][0])] # 根据频繁项的总频次排序
